...
export default function CmeForecastPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        if (!ignore) {
          setError(err.message || "Failed to load CME forecast data.");
        }
      } finally {
        if (!ignore) {
          setLoading(false);
  ...
  const stats = useMemo(() => {
    ...
    const uniqueRegions = new Set(
      rows
        .map((row) => normalizeRegion(row.active_region_num))
        .filter((value) => value !== null)
    );

    const locatedEvents = rows.filter((row) => {
      return Boolean(cleanText(row.source_location));
    }).length;
    ...
    <div style={styles.page}>
      ...
      <div style={styles.container}>
        ...
        <section style={styles.kpiGrid}>
          ...
          <KpiCard
            label="Events With Source Location"
            value={stats.locatedEvents}
          />
        </section>
        ...
        <section style={styles.panel}>
          ...
          {loading && (
            <div style={styles.messageBox}>
              <p style={styles.loadingText}>Loading CME forecast data...</p>
            </div>
          )}
          ...
          {!loading && !error && rows.length === 0 && (
            <div style={styles.messageBox}>
              <p style={styles.emptyText}>No CME forecast records found.</p>
            </div>
          )}
