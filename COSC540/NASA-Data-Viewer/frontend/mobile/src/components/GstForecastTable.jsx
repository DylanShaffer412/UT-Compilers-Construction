...
export default function GstForecastPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        if (!ignore) {
          setError(err.message || "Failed to load GST forecast data.");
        }
      } finally {
        if (!ignore) {
          setLoading(false);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      severeCount,
      latestStart: latestRow?.start_time
        ? formatDateTime(latestRow.start_time)
        : "--",
    };
  }, [normalizedRows]);
    ...
    <div style={styles.page}>
      ...
      <div style={styles.container}>
        ...
        <section style={styles.kpiGrid}>
          ...
          <KpiCard
            label="Max Kp Observed"
            value={stats.maxKp}
            danger={parseNumber(stats.maxKp) >= 7}
          />
          <KpiCard
            label="Severe Storms"
            value={stats.severeCount}
            subvalue="Kp 7 or higher"
            danger={stats.severeCount > 0}
          />
        ...
        <section style={styles.panel}>
          ...
          {loading && (
            <div style={styles.messageBox}>
              <p style={styles.loadingText}>Loading GST forecast data...</p>
            </div>
          )}
          ...
          {!loading && !error && normalizedRows.length === 0 && (
            <div style={styles.messageBox}>
              <p style={styles.emptyText}>No GST forecast records found.</p>
            </div>
          )}
                      ...
                      >
                        ...
                        <td style={styles.td}>
                          <span
                            style={{
                              ...styles.badge,
                              ...getSeverityBadgeStyle(row.maxKp),
                            }}
                          >
                            {severity}
                          </span>
...
function getKpValue(entry) {
  return (
    parseNumber(entry?.kpIndex) ??
    parseNumber(entry?.kp_index) ??
    parseNumber(entry?.kp) ??
    null
  );
}
...
function getLatestKp(entries) {
  ...
  const datedEntries = entries
    .map((entry) => {
      const observedTime =
        entry?.observedTime || entry?.observed_time || entry?.timeTag || null;

      return {
        value: getKpValue(entry),
        observedTime,
        date: observedTime ? new Date(observedTime) : null,
      };
    })
  ...
  const withValidDates = datedEntries.filter(
    (entry) => entry.date && !Number.isNaN(entry.date.getTime())
  );
...
function renderKpSamples(entries) {
  if (!entries.length) {
    return <span style={styles.muted}>No Kp data</span>;
  }

  const values = entries
    .map((entry) => getKpValue(entry))
  ...
  return (
    <div style={styles.sampleWrap}>
      ...
      {entries.length > 6 ? (
        <span style={styles.sampleMore}>+{entries.length - 6} more</span>
      ) : null}
    </div>
  );
}
...
const styles = {
  page: {
    minHeight: "100vh",
    background:
      "radial-gradient(circle at top, #102948 0%, #08111f 35%, #050b14 100%)",
    color: "#e6eef8",
    position: "relative",
    overflow: "hidden",
    fontFamily:
  ...
  scanOverlay: {
    ...
    pointerEvents: "none",
    background:
      "linear-gradient(to bottom, rgba(255,255,255,0.02) 0%, rgba(255,255,255,0.00) 50%, rgba(255,255,255,0.015) 100%)",
    opacity: 0.35,
  },
  container: {
    position: "relative",
