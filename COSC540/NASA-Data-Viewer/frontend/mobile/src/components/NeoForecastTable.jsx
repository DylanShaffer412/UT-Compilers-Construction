...
export default function NeoForecastPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        if (!ignore) {
          setError(err.message || "Failed to load NEO forecast data.");
        }
      } finally {
        if (!ignore) {
          setLoading(false);
      ...
      <div style={styles.container}>
        ...
        <section style={styles.kpiGrid}>
          ...
          <KpiCard
            label="Closest Pass"
            value={
              stats.closestDistance !== null
                ? `${formatNumber(stats.closestDistance)} mi`
                : "--"
            }
          ...
          <KpiCard
            ...
            value={
              stats.fastestVelocity !== null
                ? `${formatNumber(stats.fastestVelocity)} mph`
                : "--"
            }
        ...
        <section style={styles.panel}>
          <div style={styles.panelHeader}>
            <div>
              ...
              <p style={styles.panelText}>
                Objects are ordered by the data returned from your API.
              </p>
            </div>
          </div>
          ...
          {loading && (
            <div style={styles.messageBox}>
              <p style={styles.loadingText}>Loading NEO forecast data...</p>
            </div>
          )}
          ...
          {!loading && !error && rows.length === 0 && (
            <div style={styles.messageBox}>
              <p style={styles.emptyText}>No NEO forecast records found.</p>
            </div>
          )}
                ...
                <tbody>
                  {rows.map((row, index) => (
                    ...
                    >
                      ...
                      <td style={styles.td}>
                        {formatNumber(row.velocity_mph)} mph
                      </td>

                      <td style={styles.td}>
                        {formatNumber(row.miss_distance_miles)} mi
                      </td>
...
function normalizeNeoRow(row) {
  return {
    neo_id: row.neo_id ?? row.id ?? row.asteroid_id ?? null,
...
function formatDiameterRange(min, max) {
  ...
  if (minNum !== null && maxNum !== null) {
    return `${formatNumber(minNum)} - ${formatNumber(maxNum)} ft`;
  }
